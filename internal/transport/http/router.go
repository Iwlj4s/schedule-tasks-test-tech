...
func NewRouter(taskHandler *httphandlers.TaskHandler, scheduleHandler *httphandlers.ScheduleHandler, docsHandler *swaggerdocs.Handler) *mux.Router {
	...
	// Schedule api
	api.HandleFunc("/schedule", scheduleHandler.CreateSchedule).Methods(http.MethodPost)
	api.HandleFunc("/schedule/{id:[0-9]+}", scheduleHandler.DeleteSchedule).Methods(http.MethodDelete)
	api.HandleFunc("/schedule/{id:[0-9]+}", scheduleHandler.GetScheduleByID).Methods(http.MethodGet)
